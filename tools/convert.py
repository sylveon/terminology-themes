...
def decode_hex(hex_code):
    return tuple(int(hex_code.lstrip('#')[i:i + 2], 16) for i in (0, 2,
                 4))
...
def main():
    ...
    print_define('COL_DEF',
                 rgb_tuple_to_string(decode_hex(scheme['colorforeground'
    ...
    for (index, ansi_color) in enumerate(scheme['colorpalette'].split(';')):
        ...
        print_define('ANSI' + str(index),
                     rgb_tuple_to_string(decode_hex(ansi_color)) + ' 255')

    print ()
    print_define('BG_COLOR',
                 rgb_tuple_to_string(decode_hex(scheme['colorbackground'])) + ' 255')
    print_lines(4)

    print_define('BELL', 'ANSI1', 'Color of the bell')
    print ()

    print_define('BELL_OVERLAY_COLOR', ansi1_string + ' 100',
                 'Color flash when bell rings')
    print ()

    print_define('CURSOR',
                 rgb_tuple_to_string(decode_hex(scheme['colorcursor'])),
                 'Cursor color')
    print ()
    ...
    print_define('MEDIA_BG', 'BG_COLOR',
